...
class _ConversationsScreenState extends State<ConversationsScreen>
  ...
  void _onSearchChanged(String query) {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 300), () {
      ...
      if (query.trim().isEmpty) {
        ...
        _bloc.add(SearchConversationsEvent(query.trim()));
      }
    });
  }
  ...
  Future<void> _openChatRoom(Conversation conv) async {
    if (!_socketService.isConnected) {
      await _socketService.connect();
    }

    _socketService.checkPartnerStatus(conv.partnerId);

    if (!mounted) return;

    MessageSoundManager().setCurrentChatPartner(conv.partnerId);
  ...
  Widget _buildContent() {
    return BlocConsumer<ConversationsBloc, ConversationsState>(
      listener: (context, state) {
        if (state is ConversationsLoaded || state is ConversationsError) {
      ...
      builder: (context, state) {
        ...
        if (state is ConversationsLoaded) {
          ...
          return RefreshIndicator(
            ...
            child: _buildConversationsList(state.conversations),
          );
        }
